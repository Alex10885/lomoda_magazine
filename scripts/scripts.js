...
const getData = async () => {
    ...
    if(data.ok) {
        ...
        throw new Error(`Данные небыли получены, ошибка ${data.status} ${data.text}`);
    }
};
...
try {
    ...
    if(!goodsList){
        throw 'This is not a qoods page!';
    }
...
try {
    ...
    const cardGoodSelectWrapper = document.querySelectorAll('.card-good__select')
